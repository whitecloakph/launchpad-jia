...
import JobDropDown, { OrgType } from "@/lib/components/Dropdown/JobDropDown";
...
export default function () {
  ...
  useEffect(() => {
    async function fetchOrg() {
      ...
      let filteredOrgs = response.data.filter((org) => org.tier != "startup");
  ...
  return (
    <div className="container-fluid mt--6">
      <div className="card shadow-1">
        <div className="card-header border-0 mobile-def-table-header">
        ...
        >
          {isLoading ? (
            ...
            filteredCareers.map((job, index) => (
              <Link href={`/applicant/job-openings/${job._id}`} key={index}>
